    ...
    with tab1:
        ...
        for name in sel_one:
            with st.expander(name, expanded=True):
                ...
                if mode == "Interval":
                    # 1. RANGE SLIDER (EN ÜSTE TAŞINDI)
                    st.markdown("**Range**")
                    r_v = st.slider("", d_min, d_max, (d_min, d_max), step=1, key=f"rs_one_{name}", label_visibility="collapsed")
                    
                    # 2. ELLE GİRİŞ KUTUCUKLARI (HEMEN ALTINDA 2 SÜTUN)
                    c1_in, c2_in = st.columns(2)
                    with c1_in:
                        conf['vmin'] = st.number_input("Min", value=float(r_v[0]), key=f"nmin_one_{name}")
                    with c2_in:
                        conf['vmax'] = st.number_input("Max", value=float(r_v[1]), key=f"nmax_one_{name}")
                    ...
                    if sub == "Multi-Color":
                        ...
                        # --- İNCELTİLMİŞ PALET ÖNİZLEME (ELEGANT) ---
                        gradient = np.linspace(0, 1, 256).reshape(1, -1)
                        fig, ax = plt.subplots(figsize=(6, 0.12)) # 0.2'den 0.12'ye indirdim
                        ...
                        # One-Color seçilirse sadece renk ve opacity kalıyor
                        conf['one_c'] = st.color_picker("Color", "#FF0000", key=f"c_one_{name}")
                
                else:
                    # Threshold Modu (Aynı kaldı)
        ...
        if sel_multi:
            ...
            for name in sel_multi:
                with st.expander(f"Interval: {name}", expanded=True):
                    d_min = int(np.floor(data_objects[name].min())) if name in data_objects else 0
                    d_max = int(np.ceil(data_objects[name].max())) if name in data_objects else 100
                    r_v_m = st.slider("Range", d_min, d_max, (d_min, d_max), step=1, key=f"rs_multi_{name}")
