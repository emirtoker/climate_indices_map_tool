    ...
    with tab1:
        ...
        for name in sel_one:
            with st.expander(name, expanded=True):
                ...
                if mode == "Interval":
                    sub = st.selectbox("Sub-Mode", ["Multi-Color", "One-Color"], key=f"sub_one_{name}")
                    conf['sub_mode'] = sub
                    if sub == "Multi-Color":
                        conf['cmap'] = st.selectbox("Colormap", ["Spectral_r", "RdYlBu_r", "viridis", "magma", "YlOrRd", "Reds", "Blues"], key=f"cp_one_{name}")
                        gradient = np.linspace(0, 1, 256).reshape(1, -1)
                        fig, ax = plt.subplots(figsize=(6, 0.3)); ax.imshow(gradient, aspect='auto', cmap=plt.get_cmap(conf['cmap'])); ax.set_axis_off(); st.pyplot(fig)
                        c1, c2 = st.columns(2)
                        with c1:
                        ...
                        conf['disc'] = st.toggle("Discrete", value=True, key=f"ds_one_{name}")
                        ...
                        r_v = st.slider("Select Range", d_min, d_max, (d_min, d_max), step=1, key=f"rs_one_{name}")
                        conf['vmin'], conf['vmax'] = r_v[0], r_v[1]
                else:
                    conf['thresh'] = st.number_input("Threshold", value=float((d_min+d_max)/2), key=f"th_one_{name}")
                    col_b, col_a = st.columns(2)
                    with col_b:
                        st.markdown("**Below**")
                        conf['b_m'] = st.selectbox("Style", ["No Color", "Color"], key=f"bm_one_{name}")
                        if conf['b_m'] == "Color": conf['b_c'] = st.color_picker("Pick B-Color", "#0000FF", key=f"bc_one_{name}")
                    with col_a:
                        st.markdown("**Above**")
                        conf['a_m'] = st.selectbox("Style", ["No Color", "Color"], key=f"am_one_{name}")
                        if conf['a_m'] == "Color": conf['a_c'] = st.color_picker("Pick A-Color", "#FF0000", key=f"ac_one_{name}")
