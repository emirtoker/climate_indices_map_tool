"""
viz/map_engine.py — v2 (caption fix reverted)

This restores the original colorbar CSS that fits well visually. The
caption-wrap experiment in v3 made the colorbar narrower and pushed it
off-screen; that is reverted here. Long titles may overflow slightly,
but the visual is otherwise correct.
"""
...
def _legend_title(f: IndexFile, legend_prefix: str) -> str:
    return f"{legend_prefix}{f.display_code} - {f.long_name}"


# ===========================================================================
# STAGE 1: smart COG read
# ===========================================================================

@st.cache_data(show_spinner=False, max_entries=5)
def load_raw_data(file_path: str, max_width: int = 1200):
    import rioxarray
    ds = rioxarray.open_rasterio(file_path, mask_and_scale=True)
...
def rgba_to_png_base64(rgba_uint8) -> str:
...
@st.cache_data(show_spinner=False, max_entries=5)
def get_cached_rgba(
    file_path, vmin, vmax, cmap_input, mode,
    thresh=None, color_below=None, color_above=None,
    no_b=False, no_a=False, ext_min=False, ext_max=False,
):
    ...
    if mode == "Threshold":
        ...
        if isinstance(cmap_input, str) and cmap_input.startswith("#"):
            ...
            if ext_min:
                rgba[mask & (vals < vmin), :3] = single_c
                rgba[mask & (vals < vmin), 3] = 1.0
            if ext_max:
                rgba[mask & (vals > vmax), :3] = single_c
                rgba[mask & (vals > vmax), 3] = 1.0
        else:
            ...
            rgba[valid_range, 3] = 1.0
            if ext_min:
                rgba[mask & (vals < vmin), :3] = cmap(0.0)[:3]
                rgba[mask & (vals < vmin), 3] = 1.0
            if ext_max:
                rgba[mask & (vals > vmax), :3] = cmap(1.0)[:3]
                rgba[mask & (vals > vmax), 3] = 1.0
...
def get_synthesis_rgba(names, vmin_list, vmax_list, color):
    ...
    for i, name in enumerate(names):
        ...
        if curr.shape == ref_data.shape:
            m_ = (curr.values >= vmin_list[i]) & (curr.values <= vmax_list[i])
        else:
            reproj = curr.reindex_like(ref_data, method="nearest").values
            m_ = (reproj >= vmin_list[i]) & (reproj <= vmax_list[i])
        combined_mask = m_ if combined_mask is None else combined_mask & m_
...
def create_interactive_map(
    ...
    one_bundle,
    multi_bundle,
    index_by_filename: Dict[str, IndexFile],
    ...
    if show_osm:
        folium.TileLayer(
            ...
            attr="&copy; OSM", name="Open Street Map",
    ...
    if lcz_b64:
        ...
        m.get_root().html.add_child(folium.Element(_LCZ_LEGEND_HTML))

    # ----- Provinces --------------------------------------------------------
    if show_provinces and shp is not None:
        if "Şehir" in shp.columns:
            temp_shp = shp[["Şehir", "geometry"]].copy()
        else:
    ...
    if show_districts and districts_shp is not None:
        if "Şehir" in districts_shp.columns:
            temp_dist = districts_shp[["Şehir", "İlçe", "geometry"]].copy()
        else:
        ...
        m.add_gdf(
            ...
            labels=False, zoom_to_layer=False,
        )

    custom_legend_html = ""
    has_custom = False

    # ----- Single-Indice rendering ------------------------------------------
    if one_bundle:
        sel_one, one_conf = one_bundle
        for name in sel_one:
            if name not in index_by_filename:
                continue
            f = index_by_filename[name]
            c = one_conf.get(name)
            if c is None or not c.get("visible", True):
                continue
            ...
            rgba, bnds = get_cached_rgba(
                ...
                c.get("vmax", 1.0),
                (c.get("one_c", "#DC7933")
                 if c.get("sub_mode") == "One-Color"
                 else c.get("cmap", "viridis")),
                c["mode"],
                c.get("thresh"),
                c.get("b_c"),
                c.get("a_c"),
            ...
            clean_name = _legend_title(f, prefix)
            unit = _pretty_unit(c.get("unit", "") or f.unit)
            ...
            if c["mode"] == "Interval" and c.get("sub_mode") == "Multi-Color":
                ...
                bins = [
                    float(x) for x in
                    np.linspace(c["vmin"], c["vmax"], n_lv + 1)
                ]
                colors = [
                    mpl.colors.rgb2hex(plt.get_cmap(c["cmap"])(i))
                    for i in np.linspace(0, 1, n_lv)
                ]
                step_cm = cm.StepColormap(
                    colors, vmin=bins[0], vmax=bins[-1],
                    index=bins, caption=colorbar_title,
                )
                step_cm.tick_labels = bins
                m.add_child(step_cm)

            elif c["mode"] == "Interval":
                line = ('<div style="border-top:1px solid #ccc; margin:8px 0;"></div>'
                        if custom_legend_html else "")
                custom_legend_html += (
                    f'{line}<div style="display:flex;align-items:center;margin-bottom:8px;">'
                    f'<div style="width:20px;height:20px;background:{c["one_c"]};'
                    f'margin-right:10px; border:1px solid black;"></div>'
                    f'<span style="color:black; font-size:15px; font-weight:bold;">'
                    f'{clean_name}: {c["vmin"]:.0f}-{c["vmax"]:.0f}</span></div>'
                )
                has_custom = True
                ...
                if c.get("b_m") != "No Color":
                    rows += (
                        ...
                        f'<div style="width:20px;height:20px;background:{c.get("b_c", "#4747B5")};'
                        f'margin-right:10px; border:1px solid black;"></div>'
                        f'<span style="color:black; font-size:14px; font-weight:bold;">'
                        f'{clean_name} &lt; {thresh_val:.1f}{unit_str}</span></div>'
                    )
                if c.get("a_m") != "No Color":
                    rows += (
                        f'<div style="display:flex;align-items:center;margin-bottom:4px;">'
                        f'<div style="width:20px;height:20px;background:{c.get("a_c", "#C93131")};'
                        f'margin-right:10px; border:1px solid black;"></div>'
                        f'<span style="color:black; font-size:14px; font-weight:bold;">'
                        f'{clean_name} &gt; {thresh_val:.1f}{unit_str}</span></div>'
                    )
    ...
    if st.session_state.get("synthesis_active") and multi_bundle[0]:
        ...
        paths = tuple(index_by_filename[n].path for n in sel_multi)
        vmins = tuple(multi_conf["indices"][n]["vmin"] for n in sel_multi)
        vmaxs = tuple(multi_conf["indices"][n]["vmax"] for n in sel_multi)

        s_rgba, bnds = get_synthesis_rgba(
            paths, vmins, vmaxs, multi_conf["color"]
        )
        if s_rgba is not None:
            ...
            for i, name in enumerate(sel_multi):
                display_name = multi_conf["indices"][name].get(
                    "legend_name",
                    _legend_title(index_by_filename[name], ""),
                )
                ...
                synth_rows += (
                    ...
                    f'<div style="width:20px;height:20px;background:{color_box};'
                    f'margin-right:10px; border:{border};"></div>'
                    f'<span style="color:black; font-size:15px; font-weight:bold;">'
                    f'{display_name}: {v_min_m:.0f}-{v_max_m:.0f}</span></div>'
                )
    ...
    if has_custom:
        m.get_root().html.add_child(folium.Element(
            f'<div style="position:fixed; bottom:40px; right:10px; z-index:9999; '
            f'background:rgba(255,255,255,0.95); padding:15px; border-radius:5px; '
            f'border:1px solid #999; box-shadow: none; min-width:240px; '
            f'font-family: \'Helvetica Neue\', Arial, Helvetica, sans-serif; color: #333;">'
            f'{custom_legend_html}</div>'
        ))
