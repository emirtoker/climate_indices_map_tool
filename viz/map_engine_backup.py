...
def create_interactive_map(layers, shp, one_bundle, multi_bundle, units_dict):
    ...
    # CSS: SENİN DONDURDUĞUN MÜHÜRLÜ 16/14 PUNTO CSS
    ...
    /* Katman Kontrolü Grid Fix (3 + 1 düzeninle uyumlu) */
    ...
    .leaflet-top.leaflet-right .legend { justify-self: end !important; }
    ...
    if one_bundle:
        ...
        for name in sel_one:
            ...
            # KRİTİK: CRS HATASINI ÇÖZEN ADIM (Hata buradaydı)
            if data.rio.crs is None:
                data.rio.write_crs("EPSG:4326", inplace=True)
            
            # TR MASKELME (PhD Tıraşlaması)
            if shp is not None:
    ...
    if st.session_state.get('synthesis_active'):
        ...
        if sel_multi:
            ...
            for i, name in enumerate(sel_multi):
                ...
                # REINDEX: Farklı çözünürlükleri (DI, UTCI) hizalar
